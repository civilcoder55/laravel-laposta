...
class PostService
{
    ...
    public function store($request)
    {
        ...
        $post = auth()->user()->posts()->create(['status' => ($request->draft ? 'drafted' : 'queued'), 'message' => $request->message, 'schedule_date' => $request->schedule_date, 'logs' => $log]);
        $post->media()->attach($request->media);
        $post->accounts()->attach($request->accounts);
    }
    public function update($request, $post)
    {
        $post->update(['status' => ($request->draft ? 'drafted' : 'queued'), 'message' => $request->message, 'schedule_date' => $request->schedule_date]);
